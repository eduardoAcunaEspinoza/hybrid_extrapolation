...
class BaseConceptualModel(nn.Module):
    ...
    def map_parameters_conceptual(self, lstm_out: torch.Tensor, warmup_period:int) -> Tuple[Dict[str, torch.Tensor], 
    ...
    def _map_parameter_type(self, parameter_type:List[str]=None):
        """Returns mapping from parameter to parameter type (static or dynamic).

        The model parameters can be static or dynamic. This function creates a dictionary that associates each parameter
        ...
        map_parameter_type : Dict[str, str]
            The input dictionary, filled up with defaults for missing parameters.
        """
        map_parameter_type={}
    ...
    def _initialize_information(self, conceptual_inputs: torch.Tensor) -> Tuple[Dict[str, torch.Tensor], torch.Tensor]:
        """Initializes structures to store the time evolution of the internal states and the outflow.
        ...
        for name in self._initial_states.keys():
            states[name] = torch.zeros((conceptual_inputs.shape[0], conceptual_inputs.shape[1], 
                                        self.n_conceptual_models), dtype=torch.float32, 
