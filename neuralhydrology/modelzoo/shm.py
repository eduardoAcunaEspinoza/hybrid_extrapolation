...
class SHM(BaseConceptualModel):
    ...
    def __init__(self, cfg: Config):
        ...
        self.parameter_type = self._map_parameter_type(parameter_type=self.cfg.conceptual_model_dynamic_parameterization)
        self.n_conceptual_models = self.cfg.n_conceptual_models
    ...
    def forward(self, x_conceptual: torch.Tensor, parameters: Dict[str, torch.Tensor], 
        ...
        implemented to run multiple basins in parallel, and also multiple entities of the model at the same time. 
        ...
        initial_states : Optional[Dict[str, torch.Tensor]]
            If passed, uses these values as the initial state values for the hydrological models. If nothing is passed, 
            uses the default values defined in the property _initial_states.
        ...
        if initial_states is None: # if we did not specify initial states it takes the default values
            ss = torch.full((x_conceptual.shape[0], self.n_conceptual_models), 
                            self._initial_states['ss'], dtype=torch.float32, device=x_conceptual.device)
            sf = torch.full((x_conceptual.shape[0], self.n_conceptual_models), 
                            self._initial_states['sf'], dtype=torch.float32, device=x_conceptual.device)
            su = torch.full((x_conceptual.shape[0], self.n_conceptual_models), 
                            self._initial_states['su'], dtype=torch.float32, device=x_conceptual.device)
            si = torch.full((x_conceptual.shape[0], self.n_conceptual_models), 
                            self._initial_states['si'], dtype=torch.float32, device=x_conceptual.device)
            sb = torch.full((x_conceptual.shape[0], self.n_conceptual_models), 
                            self._initial_states['sb'], dtype=torch.float32, device=x_conceptual.device)
